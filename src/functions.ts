...
export function getBookTitlesByCategory(inputCategory: Category = Category.JavaScript): string[] {
    ...
    return books.filter(book => book.category === inputCategory).map(book => book.title); // (book => book.title) === (({title}) => title)
}
...
export function calcTotalPages() {
    ...
    let res = data.reduce((acc: bigint, obj) => acc + BigInt(obj.books) * BigInt(obj.avgPagesPerBook), 0n);
    return res;
}
